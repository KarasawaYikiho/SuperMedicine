"""Build the standalone installer EXE using PyInstaller.

Packages ``installer/gui_installer.py`` into a single-file executable
``dist/SuperMedicineInstaller.exe`` that, when double-clicked, displays
the tkinter-based GUI installation wizard.

All required data files (``install.json``, ``installer/``, ``core/``,
``permission/``, etc.) are embedded into the executable so it is fully
self-contained.

Usage from the repository root::

    python scripts/ci/build_installer_exe.py
...
# invoked as ``python scripts/ci/build_installer_exe.py``.
