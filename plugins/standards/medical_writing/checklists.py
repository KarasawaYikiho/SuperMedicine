...
CHECKLIST_VERSIONS = {"CONSORT": "2010", "STROBE": "2007"}


def _declared_checklist(name: str) -> Checklist:
    return Checklist(
        name=name,
        version=CHECKLIST_VERSIONS[name],
        items=list(CHECKLIST_ITEMS[name]),
    )


def get_consort_checklist() -> Checklist:
    """获取 CONSORT 检查清单"""
    return _declared_checklist("CONSORT")


def get_strobe_checklist() -> Checklist:
    """获取 STROBE 检查清单"""
    return _declared_checklist("STROBE")
