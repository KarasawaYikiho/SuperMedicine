...
def _numeric_columns(
    rows: list[dict[str, str]], requested: str | None
) -> dict[str, list[float]]:
    names = (
...
def missing(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        name: {
            ...
            "missing_rate": (
                sum(1 for row in rows if _is_missing(row.get(name))) / total
            )
            if total
            else 0,
        }
...
def _pearson(a: list[float], b: list[float]) -> float:
    ...
    return (
        0.0
        if da == 0 or db == 0
        else sum((x - ma) * (y - mb) for x, y in zip(a, b)) / (da * db)
    )
...
def correlation(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    return {
        f"{a}:{b}": {
            "pearson": _pearson(cols[a], cols[b]),
            "spearman": _pearson(_ranks(cols[a]), _ranks(cols[b])),
        }
...
def logistic_regression(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    if len(names) < 2:
        raise ValueError(
            "logistic-regression requires predictor and binary target columns"
        )
...
def pca(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    return {
        "columns": names,
        "explained_variance": [eig1, eig2],
        "explained_variance_ratio": [
            eig1 / trace if trace else 0,
            eig2 / trace if trace else 0,
        ],
    }
...
def kmeans(cols: dict[str, list[float]], k: int = 2) -> dict[str, Any]:
    names = list(cols)[:2]
    points = list(
        zip(cols[names[0]], cols[names[1]] if len(names) > 1 else cols[names[0]])
    )
    ...
    for _ in range(20):
        labels = [
            min(
                range(k),
                key=lambda i: (p[0] - centers[i][0]) ** 2 + (p[1] - centers[i][1]) ** 2,
            )
            for p in points
        ]
        for i in range(k):
            ...
            if assigned:
                centers[i] = [
                    sum(p[0] for p in assigned) / len(assigned),
                    sum(p[1] for p in assigned) / len(assigned),
                ]
...
def hierarchical(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    return {
        "method": "correlation-distance summary",
        "columns": names,
        "nearest_pairs": sorted(
            pairs.items(), key=lambda item: 1 - abs(item[1]["pearson"])
        )[:5],
    }
...
def time_series(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    return {
        ...
        "n": len(values),
        "mean": _mean(values),
        "first_difference_mean": _mean(diffs) if diffs else 0,
        "lag1_autocorrelation": _pearson(values[:-1], values[1:])
        if len(values) > 2
        else 0,
    }
...
def t_test(cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    if len(a) < 2 or len(b) < 2:
        raise ValueError(
            "t-test requires at least two observations in each split group"
        )
    se = math.sqrt(_variance(a) / len(a) + _variance(b) / len(b))
    stat = 0.0 if se == 0 else (_mean(a) - _mean(b)) / se
    return {
        "test": "Welch t-test normal approximation",
        "statistic": stat,
        "p_value_approx": 2 * (1 - _normal_cdf(abs(stat))),
    }
...
def chi_square(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for row in rows:
        table[(row.get(names[0], ""), row.get(names[1], ""))] = (
            table.get((row.get(names[0], ""), row.get(names[1], "")), 0) + 1
        )
    return {
        "test": "chi-square contingency summary",
        "columns": names,
        "observed": {f"{a}|{b}": v for (a, b), v in table.items()},
    }
...
def anova(rows: list[dict[str, str]], cols: dict[str, list[float]]) -> dict[str, Any]:
    ...
    ssb = sum(
        len(values) * (_mean(values) - grand) ** 2
        for values in groups.values()
        if values
    )
    ssw = sum(
        sum((v - _mean(values)) ** 2 for v in values)
        for values in groups.values()
        if values
    )
    dfb, dfw = (
        max(len(groups) - 1, 1),
        max(sum(len(v) for v in groups.values()) - len(groups), 1),
    )
    return {
        "test": "one-way ANOVA",
        "group": group_col,
        "value": value_col,
        "f_statistic": (ssb / dfb) / (ssw / dfw) if ssw else 0,
    }


def optional_wrapper(kind: str) -> dict[str, Any]:
    missing = [
        pkg for pkg in OPTIONAL_PACKAGES[kind] if importlib.util.find_spec(pkg) is None
    ]
    return {
        "algorithm": kind,
        "optional": True,
        "status": "missing_optional_dependencies" if missing else "available",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="SuperMedicine lightweight Python data-analysis algorithms"
    )
    ...
    if args.check_deps:
        print(
            "Baseline Python data-analysis actions use only the standard library; heavy ML wrappers are optional."
        )
        return 0
    try:
        payload = {
            ...
            "result": run(args.tool_kind, _read_rows(args.input), args.columns),
        }
