...
def test_tui_numeric_shortcut_9_opens_experiment_screen_and_preserves_prompt_focus(tmp_path):
    async def scenario() -> None:
        ...
        async with app.run_test(size=(140, 45)) as pilot:
            ...
            assert t("nav_experiment") in _static_text(app.query_one("#view-title", Static))
...
def test_experiment_screen_accepts_input_calculates_advances_and_saves_redacted_log(tmp_path):
    ...
    async def scenario() -> None:
        ...
        async with app.run_test(size=(140, 45)) as pilot:
            ...
            calculation_text = _static_text(app.query_one("#experiment-reagent-result", Static))
            ...
            assert t("experiment_reagent_result") in _static_text(app.query_one("#experiment-status", Static))
            ...
            assert t("experiment_step_saved") in _static_text(app.query_one("#experiment-status", Static))
            ...
            assert t("experiment_log_saved") in _static_text(app.query_one("#experiment-status", Static))
...
def test_experiment_screen_reports_missing_required_input(tmp_path):
    async def scenario() -> None:
        ...
        async with app.run_test(size=(140, 45)) as pilot:
            ...
            app.query_one("#experiment-data-input", TextArea).load_text('{"sample_id":"S1"}')
            await pilot.click("#experiment-submit")
            await pilot.pause()
