...
本文件用于把用户明确提出的要求拆分为可追踪、可验收的实现清单，并记录对应文档/界面提示覆盖情况。本文档只记录说明文字、帮助文案和实现证据映射，不改变生产功能逻辑，不包含密钥、私有端点、原始日志或运行时配置。

> 发布注意：仓库规则忽略 `docs/`，因此面向发布/验收的关键说明应优先放在根目录或其他可见可提交 Markdown 中；若引用本地 `docs/*.md`，应视为开发副本扩展说明而非发布包必需文件。
...
- [x] `REQ-TOOL-003` 向 LLM 注入工具编写规范，至少覆盖工具格式、文件/目录存储位置、命名规则、元数据和示例。证据：`core/workspace_tools.py`、`core/kernel.py`、`README.md`、工具目录 README、`tests/test_kernel.py`、`tests/test_workspace_tools.py`。
- [x] `REQ-TOOL-004` 补充 Python/R 主流数据分析算法工具或模板，覆盖常见医学科研数据分析场景。证据：`plugins/tools/python_data_analysis/`、`plugins/tools/r_data_analysis/`。
...
- [x] 实验配置存储位置/格式、LLM 新增实验配置流程：`README.md`；本地开发副本如含 `docs/experiment_config_authoring.md`，仅作为被忽略目录中的扩展说明。
- [x] Python/R 工具存储位置/格式、LLM 编写工具规则、自动扫描导入流程且用户无需知道工具 ID：`README.md`、`plugins/tools/python_data_analysis/README.md`、`plugins/tools/r_data_analysis/README.md`；本地开发副本如含 `docs/tool_authoring.md`，仅作为被忽略目录中的扩展说明。
