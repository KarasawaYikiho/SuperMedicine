...
class AuditLogger:
    ...
    def log(self, agent_id: str, action: str, resource: str, result: str, reason: str, trace_id: str | None = None) -> None:
        entry = redact_sensitive({"timestamp": datetime.now(timezone.utc).isoformat(), "trace_id": trace_id or str(uuid4()), "agent_id": agent_id, "action": action, "resource": resource, "result": result, "reason": reason, "diagnostic_stage": "audit.write", "log_path": str(self._log_path)})
