...
- Keep online/tracked categories: application and plugin source, adapters/agents/skills, installer/uninstaller code and packaging metadata, `.github/workflows/ci.yml`, `tests/`, smoke/regression assets, curated architecture/maintainer docs, release/install/security documentation other than the local-only traceability file, and canonical configuration or policy files required to run or verify the project.
